...
<script lang="ts">
	...
	$effect(() => {
		...
		const mouseTarget = (mouseReact && globalMouse ? window : currentContainer) as EventTarget;
		if (mouseReact) mouseTarget.addEventListener('mousemove', handleMouseMove as EventListener);

		return () => {
			cancelAnimationFrame(raf);
			resizeObserver.disconnect();
			if (mouseReact) mouseTarget.removeEventListener('mousemove', handleMouseMove as EventListener);
